...
WebRTC.prototype._toggleJoin = function() {
  if (!this._streaming) {
    this._gortc.start();
...
WebRTC.prototype._controlHandler = function(event) {
  ...
  if (classes(target).has(ICON_CLASS)) {
    type = target.parentElement.className;
  }

  if (!type && classes(target).has(JOIN_CLASS)) {
    type = JOIN_CLASS;
  }
  ...
  var userEl = closest(target, '.' + USER_CLASS, false);
