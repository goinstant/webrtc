...
WebRTC.prototype.initialize = function(cb) {
  ...
  this._userCache.initialize(function() {
    ...
    self._view = new View(self);
    self._controller = new Controller(self);
    ...
    if (!self._domIsBound) {
      ...
      binder.on(self._view.list, 'click', self._controller.controlHandler);

      if (self._expandContainer) {
        binder.on(self._view._expandContainer, 'click', self._controller.controlHandler);
      }
...
WebRTC.prototype.destroy = function(cb) {
  if (this._domIsBound) {
    binder.off(this._view.collapseBtn, 'click', this._view.toggleCollapse);
    binder.off(this._view.list, 'click', this._controller.controlHandler);

    if (this._expandContainer) {
      binder.off(this._view._expandContainer,
                 'click',
