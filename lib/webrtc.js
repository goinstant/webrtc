  ...
  this._userCache.initialize(function() {
    ...
    if (!self._domIsBound) {
      ...
      if (self._expandContainer) {
        binder.on(self._view.list, 'click', self._expandUser);
      }
...
WebRTC.prototype._expandUser = function(event) {
  ...
  if (!closest(target, '.' + EXPAND_CLASS, true, this._view.list)) {
    return;
  }

  // Currently expanded user to be put back into the list
  var expandedId = this._expandedUserId;

  var userEl = closest(target, '.' + USER_CLASS, false, this._view.list);
  var id = userEl.getAttribute(DATA_ID);
  var user = this._usercache.getUser(id);
  this._expandedUserId = id;

  var expandedUser = this._usercache.getUser(expandedId);

  // Get userObj and stream from container
  // if current container user is truthy addUser, addStream()
  //
  // Get userObj and stream of new expanded user
  // removeUser() from list, addUser to container
};
