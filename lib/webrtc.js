...
var PAUSE_CLASS = 'gi-pause';
var JOIN_CLASS = 'gi-join';
var LEAVE_CLASS = 'gi-leave';
...
function WebRTC(options) {
  ...
  this._expandContainer = validatedOptions.expandContainer;

  this._userCache = new UserCache(this._room);
  this._gortc = new goRTC({ room: this._room });

  this._view = new View(validatedOptions, this._userCache, this._gortc);
...
WebRTC.prototype.initialize = function(cb) {
  ...
  this._userCache.initialize(function() {
    ...
    if (!self._rtcIsBound) {
      ...
      self._gortc.on('videoOff', self._setPaused);
      self._gortc.on('videoOn', self._setResume);
    ...
    if (!self._domIsBound) {
      ...
      binder.on(self._view.list, 'click', self._controlHandler);

      if (self._expandContainer) {
        binder.on(self._view._expandContainer, 'click', self._controlHandler);
      }
...
WebRTC.prototype._setPaused = function(peer) {
  if (peer) {
    return;
  }

  var pausedKey = this._localUserKey.key(NAMESPACE).key('paused');
...
WebRTC.prototype._setResume = function(peer) {
  if (peer) {
    return;
  }
...
WebRTC.prototype._addLocalStream = function(streamObj) {
  if (this._streaming) {
    return;
  }
...
WebRTC.prototype._togglePause = function() {
  if (!this._streaming) {
    return;
  }
...
WebRTC.prototype._toggleExpand = function(target) {
  if (closest(target, '.' + LIST_CLASS, false, this._view.list)) {
    this._view.expandUser(target);

    return;
  }

  this._view.restoreUser(target);
};

WebRTC.prototype._controlHandler = function(event) {
  ...
  if (!classes(target).has(ICON_CLASS)) {
    return;
  }
  ...
  var userEl = closest(parent, '.' + USER_CLASS, false);
  var id = userEl.getAttribute(DATA_ID);

  if (classes(parent).has(AUDIO_CLASS) && id === this._localUser.id) {
    this._toggleMute();

    return;
  }

  if (classes(parent).has(PAUSE_CLASS) && id === this._localUser.id) {
    this._togglePause();

    return;
  }

  if (classes(parent).has(EXPAND_CLASS)) {
    this._toggleExpand(target);

    return;
  }

  if (classes(parent).has(LEAVE_CLASS)) {
    this._toggleStream();

    return;
  }

  var button = parent.parentElement;
  if (classes(button).has(JOIN_CLASS)) {
    this._toggleStream();
