...
View.prototype.initialize = function() {
  if (this._expandContainer) {
    classes(this._expandContainer).add(COMMON.WIDGET_CLASS);
    classes(this._expandContainer).add(COMMON.EXPAND_CONTAINER_CLASS);

    this._expandList = document.createElement('ul');
    classes(this._expandList).add(COMMON.LIST_CLASS);
    this._expandContainer.appendChild(this._expandList);
  }
  ...
  this._collapseWrapper = this._wrapper.querySelector('.' + COMMON.COLLAPSE_WRAPPER_CLASS);
  this._listWrapper = this._wrapper.querySelector('.' + COMMON.LIST_WRAPPER_CLASS);
  ...
  var localOverlayEl = localUserEl.querySelector('.' + COMMON.OVERLAY_CLASS);
...
View.prototype.addUser = function(user) {
  ...
  // Add expand controls
  if (this._expandContainer) {
    var overlayEl = userEl.querySelector('.' + COMMON.OVERLAY_CLASS);
  ...
  if (user.goinstant &&
    ...
    if (namespace.muted) {
      classes(audioEl).add(COMMON.MUTED_CLASS);
    }
...
View.prototype.removeUser = function(user) {
  ...
  var userEl = container.querySelector('[' + COMMON.DATA_ID + '="'+ id +'"]');
  ...
  switch(targetKey) {
    ...
    case COMMON.NAMESPACE + 'muted':
      ...
      if (user.goinstant.widgets.webrtc.muted) {
        ...
        classes(audioEl).remove(COMMON.MUTED_CLASS);
      }
...
View.prototype.expandUser = function(id) {
  ...
  var expandingUserEl = this.list.querySelector('[' + COMMON.DATA_ID + '="' + id + '"]');
